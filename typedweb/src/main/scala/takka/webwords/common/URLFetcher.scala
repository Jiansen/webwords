...
import scala.concurrent.{Future, Promise, promise, future, ExecutionContext}
import scala.util._
...
object URLFetcher {
    ...
    private def makeClient(implicit excutionContext: ExecutionContext) = {
    ...
    def fetchURL(u:URL)(implicit excutionContext: ExecutionContext):Future[URLFetched] = {
      val asyncHttpClient = URLFetcher.makeClient(excutionContext)
